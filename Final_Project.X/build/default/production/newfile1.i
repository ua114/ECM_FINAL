...
void FindIR(unsigned int *L_IR, unsigned int *R_IR) {
    *L_IR = ((CAP1BUFH << 8) | CAP1BUFL);
    *R_IR = ((CAP2BUFH << 8) | CAP2BUFL);
}


void IRScreen(int LeftIR,int RightIR){
        char buf[16];
...
void TrackIR(int LeftIR, int RightIR) {
    ...
    else if(LeftIR >300 && RightIR>300){
        moveMotor(&motorL, &motorR, 60,60,1,1);
        }
    else if(LeftIR <300 && RightIR<300){
        moveMotor(&motorL, &motorR, 50,50,1,0);;
        }
...
void main(void) {
    ...
    while (1)
    {
 unsigned char defuse_code;
        int character_count=0;
        defuse_code = getCharSerial();
        if (defuse_code == 0x02) {
            while (defuse_code != 0x03) {
                defuse_code = getCharSerial();
                if (character_count <12){
                    SendLCD(defuse_code, 1);
                    character_count ++;
                }
            }
        }
